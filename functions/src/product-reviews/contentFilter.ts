 ...
 */
export function flagReviewText(text: string): string[] {
  const reasons: string[] = [];
  const t = typeof text === 'string' ? text : '';
  if (!t.trim()) return reasons; // empty text is fine (rating-only review)
  if (URL_RE.test(t)) reasons.push('url');
  if (EMAIL_RE.test(t)) reasons.push('email');
