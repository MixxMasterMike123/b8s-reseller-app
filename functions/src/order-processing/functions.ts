...
function checkCampaignMatch(orderData: any, campaign: any, affiliateId: string, affiliateCode: string): boolean {
...
export async function processOrderCompletion(
  orderId: string
      ...
      const itemCount = Array.isArray((orderData as any).items) ? (orderData as any).items.length : 0;
        ...
        if (customerEmail) {
          ...
          try {
            ...
            try {
              ...
              await orderRef.update({
                emailFailures: FieldValue.arrayUnion({ type: 'ORDER_CONFIRMATION', error: msg.slice(0, 200), at: new Date().toISOString() }),
              });
            } catch (stampErr) {
          ...
          try {
            ...
            try {
              ...
              await orderRef.update({
                emailFailures: FieldValue.arrayUnion({ type: 'ORDER_NOTIFICATION_ADMIN', error: msg.slice(0, 200), at: new Date().toISOString() }),
              });
            } catch (stampErr) {
      ...
      if ((affiliate as any).shopId && (affiliate as any).shopId !== orderShopId) {
        console.error(`Affiliate ${affiliateCode} shop ${(affiliate as any).shopId} != order shop ${orderShopId}; skipping commission`);
      ...
      if (matchingCampaign && campaignShare > 0) {
        ...
        await campaignRef.update({
          totalConversions: FieldValue.increment(1),
          totalRevenue: FieldValue.increment(orderData.total || 0)
        });
