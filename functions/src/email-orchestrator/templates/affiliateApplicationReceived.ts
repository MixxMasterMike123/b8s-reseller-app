...
export function generateAffiliateApplicationReceivedTemplate(data: AffiliateApplicationReceivedData): string {
  ...
  const t = content[language as keyof typeof content] || content['sv-SE'];
...
<head>
  ...
  <style>
    ...
    .review-step {
      ...
      background: ${EMAIL_CONFIG.COLORS.BACKGROUND};
      border-radius: 8px;
      border-left: 3px solid ${EMAIL_CONFIG.COLORS.SUCCESS};
    }
    .step-number {
      background: ${EMAIL_CONFIG.COLORS.SUCCESS};
      color: white;
      width: 24px;
      height: 24px;
      border-radius: 50%;
    ...
    .thanks-message {
      font-size: 16px;
      font-weight: 600;
      color: ${EMAIL_CONFIG.COLORS.SUCCESS};
      text-align: center;
      margin: 30px 0;
      padding: 20px;
      background: #f0f9ff;
