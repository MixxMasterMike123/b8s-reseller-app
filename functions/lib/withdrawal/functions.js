...
exports.submitWithdrawal = void 0;
...
function guestRateLimitOk(ip) {
    const now = Date.now();
    // Opportunistic cleanup so the map cannot grow unbounded.
    if (guestAttempts.size > 5000) {
        for (const [k, v] of guestAttempts) {
            if (now - v.windowStart > GUEST_WINDOW_MS)
                guestAttempts.delete(k);
        }
    }
 ...
 * or with a leading '#'.
 */
async function findOrderRefByNumber(shopId, rawOrderNumber) {
 ...
 */
exports.submitWithdrawal = (0, https_1.onCall)(COMMON, async (request) => {
    ...
    const statement = request.data?.statement || {};
    const statementEmail = asTrimmed(statement.contactEmail).toLowerCase();
    if (!shopId || (!orderId && !orderNumber)) {
        throw new https_1.HttpsError('invalid-argument', 'shopId and orderId or orderNumber are required.');
    }
    ...
    if (!uid) {
        if (!orderNumber || !statementEmail) {
            throw new https_1.HttpsError('invalid-argument', 'orderNumber and contactEmail are required.');
        }
    ...
    const orderRef = orderId
        ? database_1.db.collection('orders').doc(orderId)
    ...
    const result = await database_1.db.runTransaction(async (tx) => {
        ...
        if (!ownsByUid && !ownsByVerifiedTokenEmail && !ownsByStatementEmail) {
            ...
            throw new https_1.HttpsError(uid ? 'permission-denied' : 'not-found', uid ? 'You can only withdraw your own order.' : 'Order not found.');
        }
        ...
        const acknowledgement = {
            ...
            withdrawnItems: Array.isArray(order.items)
                ? order.items.map((it) => ({ name: it?.name || '', sku: it?.sku || '', quantity: it?.quantity || 1 }))
                : [],
            consumerName: name,
            contactEmail,
            submittedAt: submittedAtIso,
            statement: `Jag, ${name}, ångrar härmed mitt köp av order ${resolvedOrderNumber}. ` +
    ...
    if (result?.eligible && result?.acknowledgement && !result?.alreadyReceived) {
        try {
            ...
            await orchestrator.sendEmail({
                emailType: 'WITHDRAWAL_ACKNOWLEDGMENT',
