            ...
            if (pi) {
                try {
                    ...
                    if (s.exists) {
                        ...
                        firebase_functions_1.logger.info('💸 dispute stamped on order', { orderId: ref.id, status: dispute.status });
                    }
                }
                catch (e) {
                    firebase_functions_1.logger.warn('⚠️ dispute stamp failed', { error: e?.message });
                }
            }
